...
class CreateNovelViewModel(
    ...
    fun updateWritingStyle(style: String) { _uiState.update { it.copy(writingStyle = style) } }
    fun updateGeneratedSettings(settings: String) { _uiState.update { it.copy(generatedSettings = settings) } }
    fun updateTargetWordCount(count: Int) { _uiState.update { it.copy(targetWordCount = count) } }
    ...
    fun generateSettings() {
        ...
        if (genre.isEmpty() || description.isEmpty()) { _uiState.update { it.copy(error = "请填写小说类型和核心构思") }; return }

        viewModelScope.launch {
            _uiState.update { it.copy(isLoading = true, step = CreateStep.GENERATING, error = null) }
            val result = grokRepository.generateNovelSettings(apiKey, preferences.model, genre, description)
            result.fold(
                onSuccess = { settings -> _uiState.update { it.copy(generatedSettings = settings, isLoading = false, step = CreateStep.REVIEW) } },
                onFailure = { e -> _uiState.update { it.copy(isLoading = false, step = CreateStep.INPUT, error = e.message ?: "生成设定失败") } }
            )
        }
    }
    ...
    fun confirmAndCreateNovel() {
        viewModelScope.launch {
            ...
            val savedNovel = novel.copy(id = id)
            memoryService.initializeCharacterStates(savedNovel, novelRepository)
    ...
    private fun extractTitle(settings: String, genre: String): String {
        ...
        for (line in lines) {
            ...
            if (trimmed.startsWith("《") && trimmed.contains("》")) return trimmed.substringAfter("《").substringBefore("》").trim()
            if (trimmed.startsWith("#")) {
                val title = trimmed.removePrefix("#").removePrefix("#").trim()
                if (title.isNotEmpty() && title.length < 30 && !title.contains("设定") && !title.contains("角色") && !title.contains("大纲")) return title
            }
        }
        return "${genre}小说"
    }
    ...
    class Factory(private val novelRepository: NovelRepository, private val grokRepository: GrokRepository, private val memoryService: MemoryService, private val preferences: AppPreferences) : ViewModelProvider.Factory {
        @Suppress("UNCHECKED_CAST")
        override fun <T : ViewModel> create(modelClass: Class<T>): T = CreateNovelViewModel(novelRepository, grokRepository, memoryService, preferences) as T
    }
}
